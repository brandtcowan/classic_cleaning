...
(function($)
{
    $.fn.dependentFields = function(options){
        var settings = $.extend({
            ...
            i;
        $(this).each(function() {
            var parentField = $(this).attr(settings.parentData).replace('[]', ''),
                showValues  = $(this).attr(settings.showValuesData).split('(?<!\\),\s*'), // don't split escaped commas
                inverseData = $(this).attr(settings.inverseData),
                that = this;
                for (var i = 0; i < showValues.length; i++) {

                    // remove backslashes before commas
                    showValues[i] = showValues[i].replace('\\', '');
                }

            $(this).find('input, textarea, select').each(function() {
                $(this).addClass(parentField + '-child');
            });
            ...
            $('*[name^="' + parentField + '"]:not(.' + parentField + '-child)').on('change ifChanged changed.bs.select', function() {
                ...
                */
                ...
                if(inverseData !== true) {
                    if(isRadio) {
                        ...
                        value = $('input[name^=' + parentField + ']:checked').val();
                    } else if(isCheckbox) {
                        value = [];
                        $('input[name^=' + parentField + ']:checked').each(function() {
                            value.push($(this).val());
                        });
                        if(value.length < 1) {
                            value = undefined;
                    ...
                    if(isRadio) {
                        ...
                        if($.inArray($('input[name^=' + parentField + ']:checked').val(), showValues) < 0) {
                            value = $('input[name^=' + parentField + ']:checked').val();
                        }
                    } else if(isCheckbox) {
                        value = [];
                ...
                // console.log(parentField + ' value = ' + value);
